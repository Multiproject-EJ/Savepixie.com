-- Pro Circles are intentionally small trusted groups, not open communities.
-- Enforce the advertised ten-active-saver cap at the same write boundary as
-- the Basic plan limits.
...
begin
  ...
  if active_members >= 10 then
    raise exception 'A SavePixie Circle can have up to ten active savers.'
      using errcode = 'P0001';
  end if;
